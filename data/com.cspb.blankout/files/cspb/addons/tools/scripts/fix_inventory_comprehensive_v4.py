...
cat_map = {
    'weapons': {'prefix': 'w', 'reset': '_reset_weap_indicators', 'ind': '_weap_use_indicator', 'exec': '_safe_exec_equip', 'img_key': 'weapons'},
...
def fix_extensions_v4():
    print("Fixing extensions in all page configs (v4)...")
...
def generate_use_files_v4():
    print("Generating unique persistence files (v4)...")
    ...
    for cat, info in cat_map.items():
        ...
        for i in range(1, 15):
            ...
            with open(page_file, 'r') as pf:
                p_content = pf.read()
                # More flexible regex for the image line
    ...
    for cat, info in cat_map.items():
        ...
        for line in lines:
            ...
            if match:
                ...
                if not slot:
                    # Fuzzy match: item_id in mapping key or vice-versa
                    for k in mapping.keys():
                        if k in item_id or item_id in k:
                            slot = mapping[k]
                            break
                if slot:
                    line = f'alias _db_{original_id}_full "{reset_alias}; alias {ind_alias} \\"exec addons/neda/persist/use/{prefix}{slot}.cfg\\"; {ind_alias}; alias _show_equip_{original_id} {exec_alias}; _show_equip_{original_id}"\n'
            new_lines.append(line)
        with open(db_path, 'w') as f: f.write("".join(new_lines))
        print(f"  Rebuilt: {db_name}_db.cfg")

def update_mapmode_control_v4():
...
fix_extensions_v4()
generate_use_files_v4()
rebuild_databases_v4()
update_mapmode_control_v4()
print("v4 Comprehensive Fix Finish.")
