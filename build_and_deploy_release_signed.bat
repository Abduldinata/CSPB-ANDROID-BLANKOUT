...
if /I "%DO_NDK_BUILD%"=="Y" (
  ...
  cd /d "%CSPB_JNI_DIR%"
  call "%NDK_BUILD%" APP_MODULES=cspb_server_android_arm64 APP_CPPFLAGS=-DCSPB_ENABLE_WEAPON_APC=%APC_VAL% -j8
  if %ERRORLEVEL% NEQ 0 (
  ...
  cd /d "%CSPB_JNI_DIR%"
  call "%NDK_BUILD%" APP_MODULES=cspb_client_android_arm64 -j8
  if %ERRORLEVEL% NEQ 0 (
