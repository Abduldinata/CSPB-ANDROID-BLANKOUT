...
static void Sys_Crash( int signal, siginfo_t *si, void *context )
{
	...
	{
		size_t remaining = ( len < (int)sizeof( message ) ) ? ( sizeof( message ) - len ) : 0;
		if( remaining > 1 )
			len += Q_snprintf( message + len, remaining, "Crash: signal %d errno %d with code %d at %p %p\n", signal, si->si_errno, si->si_code, si->si_addr, si->si_ptr );
	}
#else
	{
		size_t remaining = ( len < (int)sizeof( message ) ) ? ( sizeof( message ) - len ) : 0;
		if( remaining > 1 )
			len += Q_snprintf( message + len, remaining, "Crash: signal %d errno %d with code %d at %p\n", signal, si->si_errno, si->si_code, si->si_addr );
	}
#endif
