...
static qboolean Mod_LoadBmodelLumps( model_t *mod, byte *mod_base, size_t bufferlen, qboolean isworld )
{
	...
	for( i = 0; i < world.wadcount; i++ )
	{
		...
		if( !wadlist_warn )
		{
			size_t remaining = ( len < sizeof( wadvalue ) ) ? ( sizeof( wadvalue ) - len ) : 0;
			
			if( remaining > 1 )
			{
				ret = Q_snprintf( &wadvalue[len], remaining, "%s; ", world.wadlist[i].name );
				if( ret < 0 || (size_t)ret >= remaining )
				{
					Con_DPrintf( S_WARN "Too many wad files for output, truncating.\n" );
					wadlist_warn = true;
					if ( ret >= (int)remaining ) len = sizeof( wadvalue ) - 1;
				}
				else
				{
					len += ret;
