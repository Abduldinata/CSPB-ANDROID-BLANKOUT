...
================
*/
static void SV_SanitizePrivateDataBeforeActivate( void )
{
	edict_t *ent;
	int i;
	...
	for( i = svs.maxclients + 1; i < svgame.numEntities; i++ )
	{
		...
		if( ent->free )
		{
			...
			{
				Con_Printf( S_ERROR "Freed entity %s (%i) still has private data before ServerActivate, clearing.\n", SV_ClassName( ent ), i );
				ent->pvPrivateData = NULL;
			}
			continue;
		}

		if( !ent->v.pContainingEntity || ent->v.pContainingEntity != ent )
		{
			Con_Printf( S_WARN "Entity %s (%i) has invalid container before ServerActivate, fixing.\n", SV_ClassName( ent ), i );
			ent->v.pContainingEntity = ent;
		}

		if( ent->pvPrivateData && !Mem_IsAllocatedExt( svgame.mempool, ent->pvPrivateData ))
		{
			Con_Printf( S_ERROR "Entity %s (%i) has invalid private data %p before ServerActivate, clearing.\n", SV_ClassName( ent ), i, ent->pvPrivateData );
			ent->pvPrivateData = NULL;
		}
	}
}
...
void SV_ActivateServer( int runPhysics )
{
	...
	// Some Android runs can reach ServerActivate with stale entity private data.
	// Clear those pointers before the game DLL walks the edict list so startup
	// can skip the bad entity instead of crashing during activation.
	SV_SanitizePrivateDataBeforeActivate();
...
#if XASH_LOW_MEMORY != 2
	// CSPB Android Optimization: hard-coded to 8 (was MULTIPLAYER_BACKUP=64/128)
	// Reduces packet entity memory from ~170MB to ~22MB on arm64-v8a
	SV_UPDATE_BACKUP = ( svs.maxclients == 1 ) ? SINGLEPLAYER_BACKUP : 8;
#endif
