...
public:
	...
	// Start credits as a normal menu window.
	// This keeps the scrolling credits behavior, but doesn't trigger HostEndGame.
	void StartNormalCredits()
	{
		finalCredits = false;
		Reload();
		Show();
...
// Show credits as a normal menu window (not final credits).
// Some mods (e.g. CSPB) use this as "View Readme / Developer credits" from main menu.
void UI_Credits_Menu( void )
{
	menu_credits->Init();
	menu_credits->VidInit();
	menu_credits->StartNormalCredits();
}
