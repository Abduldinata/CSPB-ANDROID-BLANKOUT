...
void UI_ApplyServerSettings()
{
	...
	{
		if( !strcmp( var->name, "mp_gamemode" ))
		{
			// Backward compatibility:
			// old settings.scr used numeric list indexes, newer CSPB stores the real mode string.
			const char *mapped = var->value;
			if( !strcmp( var->value, "0" ) || !strcmp( var->value, "none" ))
				mapped = "tdm";
			else if( !strcmp( var->value, "1" ))
				mapped = "tdm";
			else if( !strcmp( var->value, "2" ) || !strcmp( var->value, "original" ))
				mapped = "none";
			...
			EngFuncs::CvarSetString( var->name, mapped );
		}
		else
		{
			EngFuncs::CvarSetString( var->name, var->value );
...
void UI_SetScriptCvar( const char *name, const char *value )
{
	if( menu_serveroptions && menu_serveroptions->m_pVars )
	{
		...
		scrvardef_t *var = new scrvardef_t();
		Q_strncpy( var->name, name, sizeof( var->name ) );
		Q_strncpy( var->value, value, sizeof( var->value ) );
		Q_strncpy( var->desc, name, sizeof( var->desc ) );

		if( !strcmp( name, "maxplayers" ))
		{
			var->type = T_NUMBER;
			var->number.fMin = 2.0f;
			var->number.fMax = 32.0f;
		}
		else if( !strcmp( name, "bot_quota" ))
		{
			var->type = T_NUMBER;
			var->number.fMin = 0.0f;
			var->number.fMax = 31.0f;
		}
		else
		{
			var->type = T_STRING;
		...
		if( last )
			...
			menu_serveroptions->m_pVars = var;
	}
}
