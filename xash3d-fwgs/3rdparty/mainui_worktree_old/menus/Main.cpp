...
void CMenuMain::_Init( void )
{
	...
	// CSPB shortcut: CREATE GAMES goes straight to CreateGame (map list + bot quota from settings.scr).
	...
	viewReadme.onReleased = UI_Credits_Menu;
...
void CMenuMain::VidInit( bool connected )
{
	...
	if( bCspbMenu )
	{
		// Bill-like CSPB main menu: simple vertical list on the left.
		...
		// Keep consistent with original: these are not used in CSPB menu mode.
