...
class NinjaContext(Build.BuildContext):
	...
	def execute(self):
		...
		def quote_cmd(path):  # type: (str) -> str
			if os.name == "nt" and " " in path and not (path.startswith("\"") and path.endswith("\"")):
				return "\"{}\"".format(path)
			return path

		def find_windows_python():  # type: () -> str
			local = os.environ.get("LOCALAPPDATA", "")
			candidates = []
			if local:
				candidates += glob.glob(os.path.join(local, "Python", "pythoncore-*", "python.exe"))
		...
		# App Execution Alias python.exe sometimes breaks Ninja CreateProcess on Windows.
		if os.name == "nt" and "WindowsApps" in python_cmd:
			fallback_python = find_windows_python()
			if fallback_python:
				python_cmd = fallback_python

		python_cmd = quote_cmd(python_cmd)
		...
		writer.rule(
			"waf_build",
			command="{} {} build {} {} {} --targets=$tgt".format(
				python_cmd,
