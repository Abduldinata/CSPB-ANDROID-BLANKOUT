...
int CIce::GetItemInfo(ItemInfo *p)
{
...
void CIce::Holster(int skiplocal)
{
	m_pPlayer->m_flNextAttack = UTIL_WeaponTimeBase() + 4;
SetThink(nullptr);
}

void CIce::WeaponAnimation(int iAnimation)
{
	int flags;
...
void FindHullIntersectionIcefork(const Vector &vecSrc, TraceResult &tr, float *pflMins, float *pfkMaxs, edict_t *pEntity)
{
	TraceResult trTemp;
...
void CIce::PrimaryAttack(void)
{
	...
	SetThink(&CIce::DelayedPrimaryAttack);
...
void CIce::DelayedPrimaryAttack()
{
	BOOL fDidHit = FALSE;
	UTIL_MakeVectors(m_pPlayer->pev->v_angle);
	Vector vecSrc = m_pPlayer->GetGunPosition();
	Vector vecEnd = vecSrc + gpGlobals->v_forward * 65;
...
void CIce::SetPlayerShieldAnim(void)
{
	;
}

void CIce::ResetPlayerShieldAnim(void)
{
	;
}

bool CIce::ShieldSecondaryFire(int up_anim, int down_anim)
{
	;
}

void CIce::SecondaryAttack(void)
{
	#ifndef CLIENT_DLL
	m_pPlayer->SetAnimation(PLAYER_ATTACK1);
#endif
	...
	SetThink(&CIce::DelayedSecondaryAttack2);
pev->nextthink = gpGlobals->time + 0.8;
...
void CIce::DelayedSecondaryAttack()
{
	BOOL fDidHit = FALSE;
	UTIL_MakeVectors(m_pPlayer->pev->v_angle);
	Vector vecSrc = m_pPlayer->GetGunPosition();
...
void CIce::DelayedSecondaryAttack2()
{

SetThink(&CIce::DelayedSecondaryAttack);
pev->nextthink = gpGlobals->time + 0.8;
...
void CIce::Smack(void)
{
	;
}

void CIce::SwingAgain(void)
{
	Swing(FALSE);
}

void CIce::WeaponIdle(void)
{
	ResetEmptySound();
...
int CIce::Swing(int fFirst)
{
	;
}

int CIce::Stab(int fFirst)
{
	;
}
