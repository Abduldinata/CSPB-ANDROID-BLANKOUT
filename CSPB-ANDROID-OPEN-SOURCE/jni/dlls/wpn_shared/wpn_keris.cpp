...
PRECACHE_MODEL(kKerisViewModel);
	...
	m_usKeris = PRECACHE_EVENT(1, "events/keris.sc");
}

int CKeris::GetItemInfo(ItemInfo *p)
{
...
void CKeris::Holster(int skiplocal)
{
	m_pPlayer->m_flNextAttack = UTIL_WeaponTimeBase() + 4;
SetThink(nullptr);
}

void CKeris::WeaponAnimation(int iAnimation)
{
	int flags;
	...
	PLAYBACK_EVENT_FULL(flags, ENT(m_pPlayer->pev), m_usKeris, 0, (float *)&g_vecZero, (float *)&g_vecZero, 0, 0, iAnimation, 2, 3, 4);
}

void FindHullIntersectionKeris(const Vector &vecSrc, TraceResult &tr, float *pflMins, float *pfkMaxs, edict_t *pEntity)
{
	TraceResult trTemp;
...
void CKeris::PrimaryAttack(void)
{
	...
	SetThink(&CKeris::DelayedPrimaryAttack);
...
void CKeris::DelayedPrimaryAttack()
{
	BOOL fDidHit = FALSE;
	...
	else
	{
		...
		if (pEntity)
		{
			...
			{
				EMIT_SOUND(ENT(m_pPlayer->pev), CHAN_WEAPON, "weapons/keris_hit_stab_1.wav", VOL_NORM, ATTN_NORM);
...
void CKeris::SetPlayerShieldAnim(void)
{
	;
}

void CKeris::ResetPlayerShieldAnim(void)
{
	;
}

bool CKeris::ShieldSecondaryFire(int up_anim, int down_anim)
{
	;
}

void CKeris::SecondaryAttack(void)
{
	#ifndef CLIENT_DLL
	m_pPlayer->SetAnimation(PLAYER_ATTACK1);
#endif
	...
	SetThink(&CKeris::DelayedSecondaryAttack);
	pev->nextthink = gpGlobals->time + 0.60;
...
void CKeris::DelayedSecondaryAttack()
{
	BOOL fDidHit = FALSE;
	UTIL_MakeVectors(m_pPlayer->pev->v_angle);
	Vector vecSrc = m_pPlayer->GetGunPosition();
...
void CKeris::Smack(void)
{
	;
}

void CKeris::SwingAgain(void)
{
	Swing(FALSE);
}

void CKeris::WeaponIdle(void)
{
	ResetEmptySound();
