...
PRECACHE_MODEL(kButterflyViewModel);
	...
	m_usKeris = PRECACHE_EVENT(1, "events/butterfly.sc");
}
...
void Cbutterfly::WeaponAnimation(int iAnimation)
{
	int flags;
...
void FindHullIntersectionButter(const Vector &vecSrc, TraceResult &tr, float *pflMins, float *pfkMaxs, edict_t *pEntity)
{
	TraceResult trTemp;
...
void Cbutterfly::PrimaryAttack(void)
{
	...
	SetThink(&Cbutterfly::DelayedPrimaryAttack);
...
void Cbutterfly::DelayedPrimaryAttack()
{
	BOOL fDidHit = FALSE;
...
void Cbutterfly::SetPlayerShieldAnim(void)
{
	;
}

void Cbutterfly::ResetPlayerShieldAnim(void)
{
	;
}

bool Cbutterfly::ShieldSecondaryFire(int up_anim, int down_anim)
{
	;
}
...
void Cbutterfly::SecondaryAttack(void)
{
	...
	SetThink(&Cbutterfly::DelayedSecondaryAttack);
...
void Cbutterfly::DelayedSecondaryAttack()
{
	BOOL fDidHit = FALSE;
	UTIL_MakeVectors(m_pPlayer->pev->v_angle);
	Vector vecSrc = m_pPlayer->GetGunPosition();
...
void Cbutterfly::Smack(void)
{
	;
}

void Cbutterfly::SwingAgain(void)
{
	Swing(FALSE);
}

void Cbutterfly::WeaponIdle(void)
{
	ResetEmptySound();
...
int Cbutterfly::Swing(int fFirst)
{
	BOOL fDidHit = FALSE;
	UTIL_MakeVectors(m_pPlayer->pev->v_angle);
	Vector vecSrc = m_pPlayer->GetGunPosition();
...
int Cbutterfly::Stab(int fFirst)
{
	;
}
