...
{
	ACT_RAW("Activate ENTER CFuncTankLaser::Activate");
	...
	if (!GetLaser())
	{
		ACT_RAW("Activate branch CFuncTankLaser::Activate no laser");
		UTIL_Remove(this);
		ALERT(at_error, "Laser tank with no env_laser!\n");
		ACT_RAW("Activate LEAVE CFuncTankLaser::Activate");
	}
	else
	{
		ACT_RAW("Activate branch CFuncTankLaser::Activate laser ok");
		m_pLaser->TurnOff();
		ACT_RAW("Activate LEAVE CFuncTankLaser::Activate");
	}
}
