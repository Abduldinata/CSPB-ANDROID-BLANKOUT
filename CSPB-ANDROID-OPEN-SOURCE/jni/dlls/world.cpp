...
static void CSPB_LogEdictSlotRaw( const char *tag, int entIndex )
{
	edict_t *pEdict = INDEXENT( entIndex );

	if( !pEdict )
	{
		CSPB_LOG_DIAG( "[EDICTCHK] %s ent=%d missing", tag, entIndex );
		return;
	}

	CSPB_LOG_DIAG(
		"[EDICTCHK] %s ent=%d free=%d classname_raw=%d pvPrivateData=%p",
		tag,
		entIndex,
		pEdict->free ? 1 : 0,
		(int)pEdict->v.classname,
		pEdict->pvPrivateData );
}

static void CSPB_LogStartupEdictWindow( const char *tag )
{
	CSPB_LogEdictSlotRaw( tag, 33 );
	CSPB_LogEdictSlotRaw( tag, 34 );
}
...
void CWorld::Precache()
{
	...
	CSPB_LogStartupEdictWindow("after soundent");

	InitBodyQue();
	CSPB_LogStartupEdictWindow("after InitBodyQue");

	// init sentence group playback stuff from sentences.txt.
	// ok to call this multiple times, calls after first are ignored.
	SENTENCEG_Init();
	CSPB_LogStartupEdictWindow("after SENTENCEG_Init");

	// init texture type array from materials.txt
	TEXTURETYPE_Init();
	CSPB_LogStartupEdictWindow("after TEXTURETYPE_Init");
	...
	W_Precache();
	CSPB_LogStartupEdictWindow("after W_Precache");
	ClientPrecache();
	CSPB_LogStartupEdictWindow("after ClientPrecache");
	BotPrecache();
	CSPB_LogStartupEdictWindow("after BotPrecache");
