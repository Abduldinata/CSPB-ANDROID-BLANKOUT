...
else if (sv_unlimited_reserve_ammo.value == 0)
{
...
//can't work here
/*MESSAGE_BEGIN(MSG_ONE, gmsgupdateitem, NULL, m_pPlayer->pev);
MESSAGE_END();*/
MESSAGE_BEGIN(MSG_ONE, gmsgQuickDeploy_client, NULL, m_pPlayer->pev);
MESSAGE_END();
...
MESSAGE_BEGIN(MSG_ONE, gmsgQuickReload_client, NULL, m_pPlayer->pev);
MESSAGE_END();
...
int CBasePlayerWeapon::DefaultReloadQuick(int iClipSize, int iAnim, float fDelay, int body)
{
	if (m_pPlayer->m_rgAmmo[m_iPrimaryAmmoType] <= 0)
		return FALSE;
