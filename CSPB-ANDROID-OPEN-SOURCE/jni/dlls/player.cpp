...
//trace tesr
#ifndef CLIENT_DLL
void CBasePlayer::Trace(Vector vecSrc, Vector vecDir, float flDamage, float flRadius, int bitsDamageType, entvars_t* pevInflictor, entvars_t* pevAttacker)
{

/*if ((int)CVAR_GET_FLOAT("sv_tracer_effect"))
	{
...
//damage hud
MESSAGE_BEGIN(MSG_ONE, gmsgDamagePb, NULL, pev);
MESSAGE_END();
...
//hit me
CLIENT_COMMAND(edict(), "spk vox/hit_human_3.wav\n");

//hit enemy

MESSAGE_BEGIN(MSG_ONE, gmsgHitMarker, NULL, pevAttacker);
MESSAGE_END();


int Ehealth = (int)(pev->health);
int instant;

instant = 0;

MESSAGE_BEGIN(MSG_ONE, gmsgHealthBar, NULL, pevAttacker);
WRITE_BYTE(Ehealth);
WRITE_BYTE(instant);
...
{
case 1:
//CLIENT_COMMAND(edict(), "speak \"Helmet_Hit_Defence_1\"\n");

MESSAGE_BEGIN(MSG_ONE, gmsgHelmet, NULL, pev);
MESSAGE_END();

//CLIENT_COMMAND(edict(), "speak \"helmet\"\n");
break;
...
case ACT_DEPLOY:
		{
			...
			/*if (pev->flags & FL_DUCKING)
				Q_strcpy(szAnim, "crouch_deploy_");
			else*/
				Q_strcpy(szAnim, "deploy_");
		...
		case ACT_WALK:
		{
			if ((m_Activity != ACT_RANGE_ATTACK1 || m_fSequenceFinished) && (m_Activity != ACT_DEPLOY || m_fSequenceFinished)
...
//bill: remove the star hud
MESSAGE_BEGIN(MSG_ONE, gmsgInventoryGive, NULL, pev);
MESSAGE_END();
...
void CBasePlayer::SwitchTeam()
{
	...
	if (m_iTeam == CT)
	{
		...
		case MODEL_SPETSNAZ:
			...
			break;
		case MODEL_REDFEMALE1:
			m_iModelName = MODEL_BLUEFEMALE1;
			SET_CLIENT_KEY_VALUE(entindex(), GET_INFO_BUFFER(edict()), "model", "redfemale1");
			break;
		case MODEL_REDMALE1:
			m_iModelName = MODEL_BLUEMALE1;
			SET_CLIENT_KEY_VALUE(entindex(), GET_INFO_BUFFER(edict()), "model", "redmale1");
			break;
		default:
	...
	else if (m_iTeam == TERRORIST)
	{
		...
		case MODEL_MILITIA:
			...
			SET_CLIENT_KEY_VALUE(entindex(), GET_INFO_BUFFER(edict()), "model", "spetsnaz");
				break;
		case MODEL_BLUEFEMALE1:
			m_iModelName = MODEL_REDFEMALE1;
			SET_CLIENT_KEY_VALUE(entindex(), GET_INFO_BUFFER(edict()), "model", "bluefemale1");
			break;
		case MODEL_BLUEMALE1:
			m_iModelName = MODEL_REDMALE1;
			SET_CLIENT_KEY_VALUE(entindex(), GET_INFO_BUFFER(edict()), "model", "bluemale1");
			break;
		default:
