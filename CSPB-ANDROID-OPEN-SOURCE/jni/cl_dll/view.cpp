...
void V_CalcBob ( struct ref_params_s *pparams, float freqmod, calcBobMode_t mode, double &bobtime, float &bob, float &lasttime )
{
	...
	cycle = bobtime - (int)( bobtime / 0.45 ) * 0.45;
	cycle /= 0.45;
	...
	bob = sqrt( vel[0] * vel[0] + vel[1] * vel[1] ) * 0.0024;
...
if(!pparams->onground)
    {
        lenN += pparams->frametime * 5.2;
        
        if (lenN > 1.0f)
        lenN = 1.0f;
        
        //view->origin[2] -= lenN * 0.5;
for ( i = 0; i < 3; i++ )
{
	view->origin[i] += bobRight	* 0   * pparams->right[i];
	view->origin[i] += bobUp	* 0  * pparams->up[i];
...
for(i=0;i<3;i++)
        {
        pparams->fov_x += m_IFov->value;
        pparams->fov_y += m_IFov->value;
        }
...
/*	if ( ent1->player )
	...
	// and smooth view
	V_SmoothInterpolateAngles( v_lastAngles, newAngle, angle, 90.0f );
...
void V_GetChasePos(int target, float * cl_angles, float * origin, float * angles)
{
	...
	else
	{
		...
		V_GetChaseOrigin( angles, origin, 70, origin );
	}
...
V_GetChaseOrigin( pparams->viewangles, pparams->vieworg, 50, pparams->vieworg );
...
m_IFov = gEngfuncs.pfnRegisterVariable( "ZoomFov","0", 0 );
