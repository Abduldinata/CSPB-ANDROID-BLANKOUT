...
//viewmodel_hands = IEngineStudio.GetCvar("viewmodel_hands");
...
void CStudioModelRenderer::StudioSetupBones(void)
{
	...
	for (i = 0; i < m_pStudioHeader->numbones; i++)
	{
		...
		{
			// fix knife
			if (gHUD.cl_righthand && gHUD.cl_righthand->value == 1 && bIsViewModel && IEngineStudio.IsHardware() && g_bHoldingKnife)
			{
...
else if (gHUD.cl_righthand && gHUD.cl_righthand->value == 0 && bIsViewModel && IEngineStudio.IsHardware() && g_bHoldingKnife)
{
...
void CStudioModelRenderer::DrawHands(cl_entity_t *weapon, int flags)
{
	;
}
